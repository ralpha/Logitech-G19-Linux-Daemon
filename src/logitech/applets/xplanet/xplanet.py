...
class xplanet(object):
    ...
    def __init__(self, lg19):
        ...
        if os.path.exists("/usr/bin/xplanet"):
            ...
            self.__lg19.load_text("Xplanet",1,True, center=True)
            self.__lg19.load_text("Press OK to start rendering",3)
            self.__lg19.set_text()
        else:
            self.__lg19 = lg19
            self.__lg19.load_text("Xplanet",1,True, center=True)
            self.__lg19.load_text("xplanet not installed",3, center=True)
            self.__lg19.load_text("install xplanet first!",4, center=True)
            self.__lg19.load_text("$ sudo apt-get install",5, center=True, color="red")
            self.__lg19.load_text("xplanet xplanet-images",6, center=True, color="red")
            self.__lg19.set_text()
