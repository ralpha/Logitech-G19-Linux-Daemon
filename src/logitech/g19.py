...
from g19_draw import G19DrawCanvas
...
class G19(object):
    ...
    def __init__(self, resetOnStart=False):
        ...
        fonttype  =  ImageFont.truetype ( fontPath, 20 )
        self.__canvas = G19DrawCanvas(7, fonttype);
        
        self.lastOpenApplet = self.__g19config.read("openApplet", "None")
        
        self.running = True;

    def setLastOpenApplet(self, appletName):
        self.__g19config.write("openApplet", appletName)
    ...
    def clear_text(self):
        self.__canvas.clearText();
    
    def set_text(self):
        img = self.__canvas.draw(Img.new("RGB", (320, 240)));
        self.send_frame(self.convert_text_to_image(img))
        #img.save("~/screen.png", "PNG") # take screenshots

    def load_text(self, text, line=1, clear=False, fontSize=15, center=False, color="White"):
        self.__canvas.setText(text, line, clear, center, color);
